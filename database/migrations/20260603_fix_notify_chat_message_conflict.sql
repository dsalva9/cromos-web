-- ============================================================
-- Fix notify_chat_message trigger function to match the rebuilt unique unread index
--
-- Problems solved:
--   The unique index idx_notifications_unique_unread was rebuilt to include
--   COALESCE on columns user_id, kind, listing_id, template_id, rating_id,
--   trade_id, and match_conversation_id.
--   This caused the old ON CONFLICT clause in notify_chat_message to crash
