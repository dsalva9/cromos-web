-- Migration: Add avatar_url to find_mutual_traders return type
-- Threads p.avatar_url through base → filtered → scored → final SELECT
-- Must DROP first because changing the RETURNS TABLE columns is not allowed
-- by CREATE OR REPLACE alone (Postgres error 42P13).
...
COMMENT ON FUNCTION find_mutual_traders IS 'Returns mutual trading partners with avatar_url and optional location-based scoring.';

-- Ensure permissions (must match the exact parameter signature)
