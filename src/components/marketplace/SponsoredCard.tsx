...
export function SponsoredCard({ affiliate }: SponsoredCardProps) {
        ...
        <div className="flex items-center justify-between mt-auto pt-2 border-t border-gray-100 dark:border-gray-700">
          ...
          <div className="flex items-center gap-1">
            ...
            <div className="flex items-center gap-0.5 text-amber-400">
              {Array.from({ length: Math.floor(affiliate.rating) }).map((_, i) => (
                <Star key={`full-${i}`} className="h-3 w-3 fill-current text-amber-400 stroke-amber-400" />
              ))}
              {affiliate.rating % 1 >= 0.4 && affiliate.rating % 1 <= 0.8 && (
              ...
              {Array.from({ length: 5 - Math.floor(affiliate.rating) - (affiliate.rating % 1 >= 0.4 && affiliate.rating % 1 <= 0.8 ? 1 : 0) }).map((_, i) => (
