...
export function TemplateSummaryHeader({
  ...
  return (
    <div className="relative bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden mb-8 shadow-2xl shadow-black/20">
      {/* Top Gradient Bar */}
      <div 
      ...
      <div className="p-6 md:p-8">
        <div className="flex flex-col md:flex-row gap-8 items-start justify-between">
          ...
          <div className="flex-1 space-y-4 w-full">
            ...
            <div className="space-y-3 max-w-xl">
              <div className="flex justify-between text-sm font-medium">
                ...
                <span className={isComplete ? "text-green-400" : "text-[#FFC000]"}>
                  {stats.completionPercentage}%
                </span>
              </div>
              <div className="h-4 bg-gray-800 rounded-full overflow-hidden ring-1 ring-white/5">
                <div 
                  className={`h-full rounded-full transition-all duration-1000 ease-out relative ${
                    isComplete ? 'bg-green-500' : 'bg-[#FFC000]'
                  }`}
          ...
          <div className="grid grid-cols-3 gap-2 sm:gap-3 w-full md:w-auto">
            {/* Owned */}
            <div className="bg-gray-800/40 rounded-xl p-2 sm:p-4 flex flex-col items-center justify-center border border-gray-800 hover:border-green-500/30 transition-colors group">
              <div className="p-1.5 sm:p-2 bg-green-500/10 rounded-lg mb-1 sm:mb-2 group-hover:bg-green-500/20 transition-colors">
                <Check className="h-4 w-4 sm:h-5 sm:w-5 text-green-500" />
              </div>
              <span className="text-xl sm:text-2xl font-bold text-white mb-0.5 sm:mb-1">{stats.owned}</span>
              <span className="text-[9px] sm:text-[10px] uppercase font-bold text-gray-500 tracking-wider">Tengo</span>
            </div>

            {/* Duplicates */}
            <div className="bg-gray-800/40 rounded-xl p-2 sm:p-4 flex flex-col items-center justify-center border border-gray-800 hover:border-[#FFC000]/30 transition-colors group">
              <div className="p-1.5 sm:p-2 bg-[#FFC000]/10 rounded-lg mb-1 sm:mb-2 group-hover:bg-[#FFC000]/20 transition-colors">
                <CopyIcon className="h-4 w-4 sm:h-5 sm:w-5 text-[#FFC000]" />
              </div>
              <span className="text-xl sm:text-2xl font-bold text-white mb-0.5 sm:mb-1">{stats.duplicates}</span>
              <span className="text-[9px] sm:text-[10px] uppercase font-bold text-gray-500 tracking-wider">Repes</span>
            </div>

            {/* Missing */}
            <div className="bg-gray-800/40 rounded-xl p-2 sm:p-4 flex flex-col items-center justify-center border border-gray-800 hover:border-red-500/30 transition-colors group">
              <div className="p-1.5 sm:p-2 bg-red-500/10 rounded-lg mb-1 sm:mb-2 group-hover:bg-red-500/20 transition-colors">
                <X className="h-4 w-4 sm:h-5 sm:w-5 text-red-500" />
              </div>
              <span className="text-xl sm:text-2xl font-bold text-white mb-0.5 sm:mb-1">{stats.missing}</span>
