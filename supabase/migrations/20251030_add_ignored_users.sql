-- Migration: Add ignored users functionality
...
-- Update existing RPCs to filter out ignored users

-- Update list_trade_listings to exclude ignored users
DROP FUNCTION IF EXISTS list_trade_listings(INTEGER, INTEGER, TEXT);
DROP FUNCTION IF EXISTS list_trade_listings_with_distance(INTEGER, INTEGER, TEXT, TEXT, BOOLEAN);
...
-- Update get_listing_chats to prevent messaging ignored users
DROP FUNCTION IF EXISTS get_listing_chats(BIGINT, UUID);
CREATE OR REPLACE FUNCTION get_listing_chats(p_listing_id BIGINT, p_participant_id UUID DEFAULT NULL)
RETURNS TABLE (
  id BIGINT,
  listing_id BIGINT,
  sender_id UUID,
...
BEGIN
  ...
  -- Check if current user is blocked from participating
  IF v_current_user_id IS NOT NULL AND (
    EXISTS (
      SELECT 1 FROM ignored_users iu 
      WHERE iu.user_id = v_listing_user_id 
      AND iu.ignored_user_id = v_current_user_id
    ) OR
    EXISTS (
      SELECT 1 FROM ignored_users iu 
      WHERE iu.user_id = v_current_user_id 
      AND iu.ignored_user_id = v_listing_user_id
    )
  ) THEN
    -- Return empty result if users have blocked each other
    RETURN;
  END IF;
  ...
  SELECT 
    ...
    tc.is_read,
    tc.is_system,
    tc.visible_to_user_id,
    tc.created_at,
    ps.nickname as sender_nickname,
    pr.nickname as receiver_nickname
  FROM trade_chats tc
  LEFT JOIN profiles ps ON tc.sender_id = ps.id
  LEFT JOIN profiles pr ON tc.receiver_id = pr.id
  WHERE tc.listing_id = p_listing_id
    AND (
      ...
      OR tc.is_system = TRUE
    )
    AND (
      tc.visible_to_user_id IS NULL 
      OR tc.visible_to_user_id = v_current_user_id
    )
  ORDER BY tc.created_at ASC;
END;
$$;

-- Add trigger to prevent messaging ignored users
CREATE OR REPLACE FUNCTION prevent_messaging_ignored_users()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
DECLARE
  ...
  v_receiver_id UUID;
BEGIN
  -- Get the listing owner and receiver
  SELECT user_id INTO v_listing_user_id
  FROM trade_listings
  WHERE id = NEW.listing_id;
  
  v_receiver_id := COALESCE(NEW.receiver_id, v_listing_user_id);
  
  -- Check if sender is trying to message someone who ignored them
  IF v_current_user_id IS NOT NULL AND EXISTS (
    SELECT 1 FROM ignored_users iu 
    WHERE iu.user_id = v_receiver_id 
    AND iu.ignored_user_id = v_current_user_id
  ) THEN
...
-- Create trigger
DROP TRIGGER IF EXISTS trigger_prevent_messaging_ignored_users ON trade_chats;
