-- Migration: 20260901120900_add_max_age_days_filter_to_listings_rpc.sql
-- Description: Add p_max_age_days parameter to list_trade_listings_with_collection_filter.
-- When provided (e.g. 7, 30, 90), only listings created within the last N days are returned.
-- This is especially useful as a secondary filter when sorting by distance,
-- so users only see nearby AND recent listings.

DROP FUNCTION IF EXISTS public.list_trade_listings_with_collection_filter(integer, integer, text, text, boolean, bigint[], text, boolean, bigint);
...
BEGIN
    ...
    -- Calculate minimum created_at based on max age filter
    IF p_max_age_days IS NOT NULL AND p_max_age_days > 0 THEN
        v_min_created_at := now() - (p_max_age_days || ' days')::interval;
    END IF;
    ...
    WHERE
        ...
        -- Age filter: only show listings newer than v_min_created_at
        AND (v_min_created_at IS NULL OR tl.created_at >= v_min_created_at)
        AND (
            p_slot_id IS NULL
            OR (
...
GRANT ALL ON FUNCTION public.list_trade_listings_with_collection_filter(integer, integer, text, text, boolean, bigint[], text, boolean, bigint, integer) TO anon;
GRANT ALL ON FUNCTION public.list_trade_listings_with_collection_filter(integer, integer, text, text, boolean, bigint[], text, boolean, bigint, integer) TO authenticated;
GRANT ALL ON FUNCTION public.list_trade_listings_with_collection_filter(integer, integer, text, text, boolean, bigint[], text, boolean, bigint, integer) TO service_role;
