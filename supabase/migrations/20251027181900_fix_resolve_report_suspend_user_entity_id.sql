-- =====================================================
-- FIX: Pass correct entity ID in suspend_user moderation action
-- =====================================================
-- Purpose: Fix resolve_report to pass user UUID as entity ID for suspend_user action
-- Issue: Was passing NULL, should pass the actual user ID being suspended
-- =====================================================
...
BEGIN
    ...
    ELSIF p_action = 'suspend_user' THEN
        ...
        -- FIX: Pass the user's UUID converted to a numeric representation for the audit log
        -- Since audit_log.entity_id expects BIGINT but we have UUID, we pass NULL
        -- and rely on the moderated_entity_type and old/new values to track the user
        PERFORM log_moderation_action(
            'suspend_user',
            'user',
            NULL,  -- Cannot convert UUID to BIGINT, using NULL
            p_admin_notes,
            v_old_entity,
            v_new_entity
        );
