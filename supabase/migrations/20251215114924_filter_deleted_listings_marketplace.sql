-- =====================================================
-- Filter soft-deleted listings from marketplace
-- =====================================================
-- Issue: Soft-deleted listings (deleted_at IS NOT NULL) appear in marketplace
-- Fix: Exclude deleted listings for non-admin users, include deleted_at in response
-- Changes:
--   - Add deleted_at, author_is_suspended, author_deleted_at to return type
--   - Check if viewer is admin
--   - Exclude deleted listings for non-admin users
--   - Include is_group and group_count fields
-- =====================================================
...
RETURNS TABLE(
    ...
    author_deleted_at timestamp with time zone,
...
BEGIN
    ...
    WHERE
        tl.status = 'active'
        -- Exclude soft-deleted listings for non-admin users
        AND (v_is_admin OR tl.deleted_at IS NULL)
