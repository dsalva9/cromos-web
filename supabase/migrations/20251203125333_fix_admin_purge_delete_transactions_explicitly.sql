-- Fix admin_purge_user by explicitly deleting listing_transactions BEFORE trade_listings
-- This prevents CASCADE delete from triggering unwanted behavior
...
BEGIN
    ...
    -- Critical deletion order to avoid trigger issues:
    -- 1. Delete trade_chats first (prevents system message creation)
    -- 2. Delete listing_transactions explicitly (prevents CASCADE triggers)
    -- 3. Delete trade_listings (no cascade, no triggers)
    ...
    -- 8. DELETE TRADE CHATS FIRST - prevents any system message creation
    DELETE FROM trade_chats WHERE sender_id = p_user_id OR receiver_id = p_user_id;

    -- 9. DELETE LISTING TRANSACTIONS EXPLICITLY - before listings to avoid CASCADE
    -- This way we control the deletion and prevent any triggers from firing
    DELETE FROM listing_transactions WHERE buyer_id = p_user_id OR seller_id = p_user_id;

    -- 10. Delete trade proposal items (before trade_proposals)
    DELETE FROM trade_proposal_items WHERE proposal_id IN (
        SELECT id FROM trade_proposals WHERE from_user = p_user_id OR to_user = p_user_id
    );

    -- 11. Delete trade proposals
    DELETE FROM trade_proposals WHERE from_user = p_user_id OR to_user = p_user_id;

    -- 12. Delete trade listings (after transactions are already gone, no CASCADE needed)
    ...
    -- 23. Delete audit log entries for this user (but keep admin actions for accountability)
...
'Purges all user data from the database. Requires admin permissions. Can be called with service role by providing admin_id. Uses careful deletion order: chats first, then transactions explicitly, then listings to avoid trigger conflicts.';
