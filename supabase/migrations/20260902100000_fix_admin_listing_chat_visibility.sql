-- Migration: Fix admin listing chat visibility leak
-- Removes admin bypass from SELECT WHERE clause in get_listing_chats so admins acting as buyers only see their own conversation.
...
BEGIN
    ...
    WHERE tc.listing_id = p_listing_id
      AND (
          ...
          -- Non-owner (buyer / any other user including admins interacting as buyers) sees only their own conversation with the owner
...
COMMENT ON FUNCTION public.get_listing_chats(bigint, uuid) IS 'Get chat messages for a listing - restricts access to seller and buyer, ensuring privacy for all participants';

GRANT ALL ON FUNCTION public.get_listing_chats(bigint, uuid) TO anon;
GRANT ALL ON FUNCTION public.get_listing_chats(bigint, uuid) TO authenticated;
GRANT ALL ON FUNCTION public.get_listing_chats(bigint, uuid) TO service_role;
