-- Fix admin_purge_user with simple deletion order
-- Delete chats first to avoid trigger conflicts when deleting listings/proposals
...
BEGIN
    ...
    -- Key insight: Delete trade_chats FIRST to avoid trigger issues
    ...
    -- 8. Delete listing transactions
    DELETE FROM listing_transactions WHERE buyer_id = p_user_id OR seller_id = p_user_id;

    -- 9. DELETE TRADE CHATS FIRST - this prevents the trigger issue
    -- When we delete trade_proposals or trade_listings, triggers won't try to create new chats
    ...
    -- 11. Delete trade proposals (after chats are gone)
    DELETE FROM trade_proposals WHERE from_user = p_user_id OR to_user = p_user_id;

    -- 12. Delete trade listings (after chats are gone)
...
'Purges all user data from the database. Requires admin permissions. Can be called with service role by providing admin_id. Deletes chats first to avoid trigger conflicts.';
