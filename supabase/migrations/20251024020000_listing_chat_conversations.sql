-- =====================================================
-- SPRINT 13: Listing Chat Conversations
-- =====================================================
-- Purpose: Enable full bidirectional chat for listings
-- - Sellers can reply to buyers
-- - Multiple buyers can have separate conversations
-- - Track participants and unread counts
-- =====================================================

-- Update send_listing_message to support bidirectional chat
...
-- Update get_listing_chats to support filtering by participant
...
BEGIN
    ...
    IF auth.uid() = v_listing_user_id THEN
        ...
        ELSE
            -- Viewing all conversations (shouldn't happen in UI, but allowed)
            RETURN QUERY
            SELECT
                tc.id,
                tc.sender_id,
...
-- Create function to get chat participants for a listing
...
CREATE OR REPLACE FUNCTION mark_listing_messages_read(
    p_listing_id BIGINT,
    p_sender_id UUID
)
RETURNS INTEGER
...
GRANT EXECUTE ON FUNCTION get_listing_chat_participants TO authenticated;
GRANT EXECUTE ON FUNCTION mark_listing_messages_read TO authenticated;
...
-- TODO Sprint 15: Add realtime trigger for notifications
-- CREATE OR REPLACE FUNCTION notify_listing_chat_message()
-- RETURNS TRIGGER AS $$
-- BEGIN
