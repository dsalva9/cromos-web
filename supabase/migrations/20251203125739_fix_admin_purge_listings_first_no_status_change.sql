-- Fix admin_purge_user by deleting listings IMMEDIATELY after chats
-- The listing DELETE cascades to transactions without triggering status changes
-- This is the key: listings CASCADE delete to transactions, not the other way around
...
BEGIN
    ...
    -- CRITICAL ORDER:
    -- 1. Delete trade_chats first (no messages can be created)
    -- 2. Delete trade_listings immediately (CASCADE deletes transactions, no status change)
    ...
    -- 9. DELETE TRADE LISTINGS IMMEDIATELY AFTER CHATS
    -- This uses CASCADE to delete listing_transactions WITHOUT changing listing status
    -- Key insight: Deleting the listing directly doesn't trigger status change logic
    DELETE FROM trade_listings WHERE user_id = p_user_id;
    -- Note: listing_transactions are CASCADE deleted here
...
'Purges all user data from the database. Requires admin permissions. Can be called with service role by providing admin_id. Deletes chats first, then listings (cascade to transactions) to avoid any status change triggers.';
