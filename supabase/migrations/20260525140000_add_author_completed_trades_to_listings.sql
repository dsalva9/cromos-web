-- Migration to add author_completed_trades to list_trade_listings_with_collection_filter RPC function
...
-- Recreate the function with returns TABLE including author_completed_trades
...
RETURNS TABLE (
  ...
  is_group boolean,
  group_count integer,
  author_completed_trades integer -- New Column!
)
LANGUAGE plpgsql
SECURITY DEFINER
AS $function$
...
BEGIN
    ...
    SELECT
        ...
        tl.is_group,
        tl.group_count,
        p.completed_trades AS author_completed_trades -- Select completed_trades
