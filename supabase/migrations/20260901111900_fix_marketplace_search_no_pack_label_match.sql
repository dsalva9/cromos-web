-- Migration: 20260901111900_fix_marketplace_search_no_pack_label_match.sql
-- Description: Remove label/page_title matching from listing_pack_items in text search.
-- Pack listings should only match by their own title/description/sticker_number,
-- NOT by individual item labels. This prevents "Pack de Repes" listings from
-- appearing when searching for a specific sticker name like "Ramy Bensebaini".
-- Pack items can still be found by slot_number (numeric search).
