-- Migration: Update Notification Base URL to cambiocromos.com in production
-- Created: 2026-07-14
...
DECLARE
    ...
    v_base_url TEXT := 'https://cambiocromos.com';
BEGIN
    ...
    CASE NEW.kind
        ...
        WHEN 'match_chat_message' THEN
            v_title := '⚡ Nuevo match';
            v_body := COALESCE(NEW.payload->>'message_preview', 'Alguien quiere intercambiar cromos contigo');
            v_action_url := v_base_url || '/chats?tab=match';
        WHEN 'trade_confirmation_request' THEN
            v_title := 'Confirma un intercambio';
            v_body := 'Tienes una solicitud para confirmar un intercambio';
            v_action_url := v_base_url || '/intercambios';
        WHEN 'trade_confirmed' THEN
            v_title := 'Intercambio confirmado';
            v_body := 'El intercambio ha sido confirmado por la otra parte';
            v_action_url := v_base_url || '/intercambios';
        ELSE
