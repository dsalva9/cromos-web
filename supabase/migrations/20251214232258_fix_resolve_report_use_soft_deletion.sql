-- =====================================================
-- Fix resolve_report to use soft deletion for data retention
-- =====================================================
-- Issue: Removed content was using status='removed' instead of deleted_at
-- Fix: Use deleted_at timestamp for proper data retention (90-day soft delete)
-- Changes:
--   - Listings: Set deleted_at instead of status='removed'
--   - Templates: Set deleted_at AND is_public=FALSE
--   - User suspension: Soft delete user's listings with deleted_at
-- =====================================================
...
BEGIN
    ...
    ELSIF p_action = 'suspend_user' THEN
        ...
        BEGIN
            v_target_uuid := v_report.target_id::uuid;
        EXCEPTION WHEN invalid_text_representation THEN
        ...
        FROM trade_listings
        WHERE user_id = v_target_uuid AND status = 'active' AND deleted_at IS NULL;
