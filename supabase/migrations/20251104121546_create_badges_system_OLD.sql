...
-- 3. Updates to user_badges table
...
CREATE INDEX idx_badge_definitions_sort_order ON badge_definitions(sort_order);
...
CREATE INDEX idx_user_badge_progress_category ON user_badge_progress(badge_category);
...
ALTER TABLE user_badges
    ADD COLUMN IF NOT EXISTS badge_id TEXT REFERENCES badge_definitions(id),
...
-- Trigger function to increment progress and check for badge award
CREATE OR REPLACE FUNCTION trigger_check_badge_on_progress()
RETURNS TRIGGER AS $$
DECLARE
    v_user_id UUID;
    v_category TEXT;
BEGIN
    -- Extract user_id and category from the triggering table
    -- This will be customized per trigger

    -- Default: just return NEW
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    -- Increment progress
    v_new_count := increment_badge_progress(NEW.creator_id, 'creator');

    -- Check and award badge
    PERFORM check_and_award_badge(NEW.creator_id, 'creator');
...
-- This trigger fires when a collection is marked as complete
...
BEGIN
    -- Only process if completion_status changed to true
    IF NEW.completion_status = TRUE AND (OLD.completion_status IS NULL OR OLD.completion_status = FALSE) THEN
        -- Increment progress
...
CREATE TRIGGER trigger_completionist_badge_on_complete
    AFTER UPDATE ON user_template_progress
    FOR EACH ROW
    WHEN (NEW.completion_status = TRUE)
...
-- Add badge_earned to notification kinds if not exists
DO $$
BEGIN
    -- This assumes you have a notification_kind type or check constraint
    -- Adjust based on your actual notifications table structure

    -- If using ENUM type, you'd need to alter it
    -- ALTER TYPE notification_kind ADD VALUE IF NOT EXISTS 'badge_earned';

    -- If using TEXT with CHECK constraint, update the constraint
    -- (This will be handled in your notifications table structure)
END $$;
...
-- User badge progress: system can insert/update (handled by SECURITY DEFINER functions)
-- No direct INSERT/UPDATE policies needed as all operations go through RPC functions
