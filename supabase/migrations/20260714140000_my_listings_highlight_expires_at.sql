-- Migration: Add highlight_expires_at to get_my_listings_with_progress RPC
-- This updates the function to return the expiry timestamp of the active highlight.
...
BEGIN
    ...
    SELECT
        ...
        (SELECT lh.expires_at FROM listing_highlights lh
            WHERE lh.listing_id = tl.id
            AND lh.expires_at > now()
            ORDER BY lh.expires_at DESC
            LIMIT 1
        ) AS highlight_expires_at
